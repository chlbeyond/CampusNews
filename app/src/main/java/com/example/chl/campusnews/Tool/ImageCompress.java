...
//	 * @param path
	 ...
	 * @return
	 */
	public static int[] calculateImageSize(Options options, int resW, int resH) {
		final int height = options.outHeight;
		final int width = options.outWidth;
		Log.i("image_height", height + "");
		Log.i("image_width", width + "");

		float sizeRatio = 1;
		if (resW > 0 && resH > 0) {
			if (height > resH || width > resW) {
				float heightRatio = (float) height / (float) resH;
				float widthRatio = (float) width / (float) resW;
				sizeRatio = heightRatio > widthRatio ? heightRatio : widthRatio;
			}
		}
		...
		if (sizeRatio <= 1) {
			...
			newHeight = Math.round((float) height / sizeRatio);
			newWidth = Math.round((float) width / sizeRatio);
		}
	...
	public final static Bitmap compressBitmap(String path, int rqsW, int rqsH) {
		  final Options options = new Options();
	        ...
	        options.inSampleSize = calculateInSampleSize(options, rqsW,
	        		rqsH);
...
//			Log.e("BitmapHelper-->compressBitmap", e.getMessage() + "");
	...
	//加锁防止多次调用造成内存溢出
	public synchronized  final static String compressBitmap(Context context, String srcPath, int rqsW, int rqsH, long maxByte) {
		Bitmap bitmap = compressBitmap(compressBitmap(srcPath, rqsW, rqsH), maxByte);
		File srcFile = new File(srcPath);
		String desPath = getImageCacheDir(context) + srcFile.getName();
		int degree = getDegress(srcPath);
		try {
...
//			Log.e("BitmapHelper-->compressBitmap", e.getMessage() + "");
	...
	/**
	 * 此方法过期，该方法可能造成OutOfMemoryError，使用不含isAdjust参数的方法 get bitmap form resource
	 * directory, and then compress bitmap according to reqsW and reqsH
	 * 
	 * @param res
	 *            {@link Resources}
	 * @param resID
	...
	/**
	 * 基于质量的压缩算法， 此方法未 解决压缩后图像失真问题 可先调用比例压缩适当压缩图片后，再调用此方法可解决上述问题
	 * 
//	 * @param bts
	 * @param maxBytes
	 *            压缩后的图像最大大小 单位为byte
	 * @return
	 */
	public synchronized final static Bitmap compressBitmap(Bitmap bitmap, long maxBytes) {

		 ByteArrayOutputStream baos = new ByteArrayOutputStream();
		 bitmap.compress(CompressFormat.JPEG, 50, baos);// 质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
	     int options = 100;
	        while (baos.toByteArray().length > maxBytes) { // 循环判断如果压缩后图片是否大于(maxkb)50kb,大于继续压缩
//	          Log.i(test,压缩一次!);
	 ...
	 * { try { ByteArrayOutputStream baos = new ByteArrayOutputStream(); byte[]
	 * bts = new byte[1024]; while (is.read(bts) != -1) baos.write(bts, 0,
	 * bts.length); is.close(); int options = 100; while
	 * (baos.toByteArray().length > maxBytes) {
	 * 
	...
	private static String getImageCacheDir(Context context) {
		String dir = FileHelper.getCacheDir(context) + "compress" + File.separator;
